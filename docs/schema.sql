...
-- ==========================================
-- SHARE VIEWS (audit log lượt xem trang chia sẻ)
-- ==========================================
...
-- Ghi 1 lượt xem (anon gọi được); lazy cleanup ~5% số lần gọi (>30 ngày)
CREATE OR REPLACE FUNCTION public.log_share_view(
  p_token TEXT, p_ip TEXT, p_ua TEXT, p_city TEXT, p_referrer TEXT, p_device TEXT
) RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp AS $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.shared_links WHERE token = p_token AND expires_at > NOW()) THEN RETURN; END IF;
  INSERT INTO public.share_views (token, ip, user_agent, city, referrer, device_type) VALUES (p_token, p_ip, p_ua, p_city, p_referrer, p_device);
  IF random() < 0.05 THEN DELETE FROM public.share_views WHERE viewed_at < NOW() - INTERVAL '30 days'; END IF;
END; $$;

-- Tổng hợp lượt xem theo token (chỉ admin)
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'admin') THEN RAISE EXCEPTION 'Access denied.'; END IF;
  RETURN QUERY SELECT sv.token, COUNT(*)::BIGINT, MAX(sv.viewed_at) FROM public.share_views sv GROUP BY sv.token;
END; $$;

-- Nhật ký chi tiết 1 link (chỉ admin)
CREATE OR REPLACE FUNCTION public.get_share_views(p_token TEXT, p_limit INT DEFAULT 100)
RETURNS TABLE(viewed_at TIMESTAMPTZ, ip TEXT, user_agent TEXT, city TEXT, referrer TEXT, device_type TEXT)
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp AS $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'admin') THEN RAISE EXCEPTION 'Access denied.'; END IF;
  RETURN QUERY SELECT sv.viewed_at, sv.ip, sv.user_agent, sv.city, sv.referrer, sv.device_type FROM public.share_views sv WHERE sv.token = p_token ORDER BY sv.viewed_at DESC LIMIT p_limit;
END; $$;
