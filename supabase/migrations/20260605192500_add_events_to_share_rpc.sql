-- redrop & recreate get_shared_family_tree to return custom_events and more person details
...
BEGIN
  ...
  -- Xây dựng JSON trả về chứa danh sách thành viên, mối quan hệ và các sự kiện dòng họ
  SELECT jsonb_build_object(
    'settings', COALESCE(link_rec.settings, '{}'::jsonb),
    'persons', (
      SELECT COALESCE(jsonb_agg(p), '[]'::jsonb)
      FROM (
        -- Lấy đầy đủ thông tin phục vụ tính toán ngày giỗ/sinh nhật ở chế độ client-side
